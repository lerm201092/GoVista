...
<div id="navbarDefault">
  <div class="container-fluid">
    ...
    <div class="navbar-header">
	  ...
	  <i style="color:#fff !important;position: relative !important;top: 5px !important;" class="material-icons">domain</i><span style="font-size: 14px !important;font-weight: bold !important;color: #fff !important;">{{ Session::get('id_empresa_nombre') }}</span>
       </div>
      ...
      <ul class="nav navbar-nav navbar-right">	  
        <li class="nav-item"> 
                <a title="Total Sesiones" class="nav-link" href="javascript:void(0);" id="navbarDropdownMenuLink" aria-haspopup="true" aria-expanded="false">
                  ...
                  <span style="border: 1px solid #757575;background: white !important;color: black !important;font-size: 11px !important;" class="notification">{{ (auth()->user()->total_sessiones != null)? auth()->user()->total_sessiones : '0'}}</span>                 
                </a>               
              </li>
			  <li class="nav-item">   
                <a title="{{ (\Cart::count() > 0)? 'Ir a Carrito De Compra' : 'Carrito Vacio' }}" class="nav-link" href="{{ route('viewcart') }}" id="navbarDropdownMenuLink" aria-haspopup="true" aria-expanded="false">
                  <i style="color:#fff !important;font-size:18pt !important;" class="material-icons">shopping_cart</i>
                  <span style="border: 1px solid #757575;background: white !important;color: black !important;font-size: 11px !important;" class="notification">{{ \Cart::count() }}</span>                 
                </a>               
              </li>
        <li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">
							@if (auth()->user()->image)
								<img src="{{ auth()->user()->image }}" style="width: 24px; height: 24px; border-radius: 50%;">
							@else
								<i style="color:#fff !important;font-size:18pt !important;" class="material-icons">account_circle</i>
								@endif		
							<span style="color:#fff !important;" >{{ Auth::user()->nombres.' '.Auth::user()->apellidos }}</span><span style="color:#fff !important;" class="caret"></span>								
                            </a>
          <ul class="dropdown-menu">
            <li>
                                <a href="{{ route('profile') }}"> <i class="material-icons">account_box</i> Mi Perfil</a>
            </li>
            <li>
                                <a href="{{ route('changePassword') }}"> <i class="material-icons">security</i> Cambiar Contraseña</a>
            </li>
			<li>
                                <a href="{{route('productList')}}"> <i class="material-icons">shopping_cart</i> Comprar</a>
            </li>
			<li>
                                <a href="{{route('detailspayments')}}"> <i class="material-icons">payment</i> Detalles De Pagos</a>
            </li>
...
<script src="{{ asset('assets/js/chartist-plugin-axistitle.min.js') }}" type="text/javascript"></script>
