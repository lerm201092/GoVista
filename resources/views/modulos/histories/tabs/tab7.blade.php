...
<label style="line-height: unset !important;" class="containerInput">
  ...
  <input id="id_exercise_{{ $e->id }}" name="id_exercise[]" class="form-check-input" type="checkbox" value="{{ $e->id }}" checked>  
  @else
  <input id="id_exercise_{{ $e->id }}" name="id_exercise[]" class="form-check-input" type="checkbox" value="{{ $e->id }}">  
  @endif  
  ...
  <span class="checkmark"></span>
      ...
      <td>	      
	  <div class="form-group label-floating">		    		
	  <input id="useExercise_{{ $e->id }}" name="useExercise[]" rel="tooltip" title="{{ $e->useExercise }}" class="form-control" readonly type="text" value="{{ $e->useExercise }}">
      </div>
	  </td>
      <td>
	  <div class="form-group label-floating">
	...
	@endif
	  ...
	  <div class="form-group label-floating">	  
	  @if ( count($historyExerciseFill->where('id_exercise', '=', $e->id)) > 0) 
       <input id="session_{{ $e->id }}" name="session[]" style="text-align: center !important;width: 80px;" class="form-control" min="1" max="10" type="number" value='{{ $historyExerciseFill->where('id_exercise', '=', $e->id)->first()->session }}'>
		@else
		<input id="session_{{ $e->id }}" name="session[]" style="text-align: center !important;width: 80px;" class="form-control" min="1" max="10" type="number" value=''>
 	@endif 	
	  </div>
	  </td>
      <td style="width: auto !important;">
	  <div class="form-group label-floating">
@if ( count($historyExerciseFill->where('id_exercise', '=', $e->id)) > 0) 
		<select class="form-control" id="size_{{ $e->id }}" name="size[]">	        
...
@endif		                 
	...
	@endif 
     ...
     <td style="width: auto !important;">
	  <div class="form-group label-floating">	  	
		@if ( count($historyExerciseFill->where('id_exercise', '=', $e->id)) > 0) 
			<input id="duration_{{ $e->id }}" name="duration[]" style="text-align: center !important;width: 80px;" class="form-control" min="5" max="30" step="5" type="number" value='{{ $historyExerciseFill->where('id_exercise', '=', $e->id)->first()->duration }}'>
		@else
		<input id="duration_{{ $e->id }}" name="duration[]" style="text-align: center !important;width: 80px;" class="form-control" min="5" max="30" step="5" type="number" value=''>
		@endif 			
	  </div>
	  </td>
    </tr>  
